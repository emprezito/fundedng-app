...
import {
  verifyKycServer,
  verifyKycDocumentServer,
  rejectKycDocumentServer,
} from "@/server/kyc.functions";
import {
...
function useAdminDataHook() {
  ...
  const [partnerFreeForm, setPartnerFreeForm] = useState({
    login: "",
    password: "",
    investor: "",
    server: "",
  ...
  const openEditChallenge = (c: any) => {
    ...
    setChallengeForm({
      ...c,
      restricted_symbols: Array.isArray(c.restricted_symbols)
        ? c.restricted_symbols.join(", ")
        : "",
    });
    setChallengeEditOpen(true);
  };
  ...
  const saveChallenge = async () => {
    ...
    const payload: any = {
      ...
      category: ["classic", "titan", "flash"].includes(challengeForm.category)
        ? challengeForm.category
        : "classic",
      restricted_symbols: String(challengeForm.restricted_symbols || "")
        .split(",")
        .map((s: string) => s.trim())
        .filter(Boolean),
    };
  ...
  const deleteChallenge = async (c: any) => {
    ...
    if ((orderCount ?? 0) > 0 || (accountCount ?? 0) > 0) {
      ...
      const refs = [
        ...
        accountCount ? `${accountCount} account(s)` : "",
      ]
        .filter(Boolean)
        .join(" and ");
  ...
  const load = async () => {
    ...
    const [profRes, chRes, ordRes, taRes] = await Promise.all([
      userIds.length
        ? supabase
            .from("profiles")
            .select(
              "id, full_name, bank_account_number, bank_name, bank_account_name, kyc_verified",
            )
            .in("id", userIds)
      ...
      challengeIds.length
        ? supabase
            .from("challenges")
            .select(
              "id, name, account_size, profit_target_percent, phase2_profit_target_percent, max_drawdown_percent, phases, drawdown_type",
            )
            .in("id", challengeIds)
        : Promise.resolve({ data: [] as any[] }),
      orderIds.length
        ? supabase
            .from("orders")
            .select(
              "id, status, reset_account_id, utm_source, utm_medium, utm_campaign, utm_content",
            )
            .in("id", orderIds)
        : Promise.resolve({ data: [] as any[] }),
      accountIds.length
        ? supabase
            .from("trader_accounts")
            .select(
              "id, mt5_login, currency, starting_balance, monitor_paused, monitor_paused_reason",
            )
            .in("id", accountIds)
        : Promise.resolve({ data: [] as any[] }),
    ]);
    ...
    const accList = accRows.map((a: any) => ({
      ...a,
      profiles: profMap.get(a.user_id) ?? null,
      challenges: chMap.get(a.challenge_id) ?? null,
      _trading_days: a.trading_days ?? 0,
    }));
    const accMap = new Map(accList.map((a: any) => [a.id, a]));
    const poList = poRows.map((p: any) => ({
      ...p,
      profiles: profMap.get(p.user_id) ?? null,
      trader_accounts: accMap.get(p.trader_account_id) ?? taMap.get(p.trader_account_id) ?? null,
    }));
    const hydrated = reqRows.map((r: any) => ({
      ...r,
    ...
    const passedCount = accList.filter(
    ...
    setStats({
      ...
      paid: poList
        ...
        .reduce((s: number, p: any) => s + Number(p.amount_naira), 0),
      passRate,
    });
    try {
      const { data } = await supabase.rpc("find_unprovisioned_orders" as never);
      setUnprovisionedOrders((data as any[]) ?? []);
    } catch {
  ...
  const deleteRequest = async (req: any) => {
    if (
      !confirm(
        `Delete this pending request for ${req.profiles?.full_name ?? "trader"}? This will also cancel the order.`,
      )
    )
      return;
    const { data: sess } = await supabase.auth.getSession();
  ...
  const updatePayout = async (p: any, status: "approved" | "paid" | "rejected") => {
    ...
    if (status === "paid") {
      ...
      const res = await provisionPayoutServer({
        data: { accessToken: sess.session.access_token, payoutId: p.id },
      });
      if (res?.ok) {
    ...
    if (status === "rejected")
      notifyEmail({ type: "payout_rejected", payoutId: p.id, reason: "Rejected by admin." });
    load();
  };
  ...
  const resetAccountBalance = async (account: any) => {
    if (
      !confirm(
        `Reset balance for ${account.profiles?.full_name ?? "trader"} (${account.mt5_login})? Equity will reset to ${formatNaira(account.starting_balance)}.`,
      )
    )
      return;
    const { error } = await supabase
  ...
  const approvePhase2 = async (a: any) => {
    ...
    if (
      !confirm(
        `Approve Phase 2 for ${a.profiles?.full_name ?? "trader"}? A new account will be provisioned from the pool.`,
      )
    )
      return;
    if (!session?.access_token) return toast.error("Please sign in again");
    try {
      const result = await approvePhase2Server({
        data: { accessToken: session.access_token, accountId: a.id },
      });
  ...
  const approveFunded = async (a: any) => {
    ...
    if (
      !confirm(
        `Approve Funded status for ${a.profiles?.full_name ?? "trader"}? A new funded account will be provisioned from the pool.`,
      )
    )
      return;
    if (!session?.access_token) return toast.error("Please sign in again");
    try {
      const result = await approveFundedServer({
        data: { accessToken: session.access_token, accountId: a.id },
      });
      if (!result?.ok) return toast.error(result?.error ?? "Approval failed");
  ...
  const advanceTier = async (a: any) => {
    if (a.status !== "funded" && Number(a.current_phase) < 3)
      return toast.error("Only funded accounts can advance a tier");
    const from = Number(a.funded_tier ?? 1);
    const to = from + 1;
    if (
      !confirm(
        `Advance ${a.profiles?.full_name ?? "trader"} (${a.mt5_login}) from Funded ${from} to Funded ${to}? This will close their current account and provision a fresh one from the Funded ${to} pool.`,
      )
    )
      return;
    if (!session?.access_token) return toast.error("Please sign in again");
    try {
      const result = await provisionNextTierServer({
        data: { accessToken: session.access_token, traderAccountId: a.id },
      });
  ...
  const submitEquity = async (account: any) => {
    ...
    if (!Number.isFinite(equity) || equity < 0)
      return toast.error("Equity must be a positive number");
  ...
  const openKycVerify = (account: any) => {
  ...
  const submitKycVerify = async () => {
    ...
    try {
      ...
      if (hasDoc) {
        const res = await verifyKycDocumentServer({
          data: { userId: kycTarget.user_id, accessToken: session.access_token },
        });
        if (!res?.ok) {
  ...
  const submitKycReject = async () => {
    ...
    try {
      const res = await rejectKycDocumentServer({
        data: {
          userId: kycTarget.user_id,
          reason: kycRejectReason.trim(),
  ...
  const openBreachDialog = (account: any) => {
    ...
    setBreachDuration("");
    setBreachReason(
      `Hi ${name}, your FundedNG challenge account has been closed due to inactivity. Our rules require at least 1 trade every calendar week to keep your account active. Unfortunately no trading activity was detected on your account within the required period.\nYou're welcome to start a new challenge anytime at fundedng.fun 💪\n— FundedNG Team`,
    );
  };
  const openWarningDialog = (account: any) => {
    ...
    setWarnDuration("");
    setWarnReason(
      `Hi ${name}, your FundedNG challenge account is at risk of being closed due to inactivity. Our rules require at least 1 trade every calendar week to keep your account active. Please place a trade to keep your account active.\n— FundedNG Team`,
    );
  };
  ...
  const openPayoutRejectDialog = (payout: any) => {
  ...
  const submitPayoutReject = async () => {
    ...
    try {
      const { error } = await supabase
        .from("payouts")
        .update({ status: "rejected", processed_at: new Date().toISOString(), admin_note: reason })
        .eq("id", payoutRejectTarget.id);
      if (error) {
  ...
  const loadAffiliate = () => {
    (async () => {
      const [pRes, cRes, affRes, refRes, commRes, ordRes, chRes] = await Promise.all([
        ...
        supabase
          .from("orders")
          .select("id, user_id, amount_paid, challenge_id, status")
      ...
      setFreeClaims(
        claimRows.map((r: any) => ({ ...r, profiles: profMap.get(r.affiliate_id) ?? null })),
      );
      ...
      const statsArr = affRows.map((a: any) => {
        ...
        const paidOrders = ordRows.filter((o) =>
          refs.some((r) => r.referred_user_id === o.user_id),
        );
        const revenue = paidOrders.reduce(
          (s: number, o: any) => s + Number(o.amount_paid) / 100,
          0,
        );
        ...
        return {
          ...
          pendingCommissions: comms
            ...
            .reduce((s: number, c: any) => s + Number(c.amount_naira), 0),
    ...
    (async () => {
      const [pRes, payRes, freeRes] = await Promise.all([
        ...
        (supabase as any)
          ...
          .select(
            "*, challenges(name, account_size, profit_target_percent, max_drawdown_percent, phases, currency)",
          )
      ...
      const challengeIds = Array.from(
        new Set(partnerRows.map((r) => r.free_account_challenge_id).filter(Boolean)),
      );
      const profMap = new Map<string, any>();
      if (userIds.length) {
        const { data: profs } = await supabase
      ...
      setPartners(
        partnerRows.map((r: any) => ({
          ...r,
          profiles: profMap.get(r.user_id) ?? null,
          free_challenge: r.free_account_challenge_id
            ? (chMap.get(r.free_account_challenge_id) ?? null)
            : null,
        })),
      );
      setPartnerPayouts(
        payRows.map((r: any) => ({ ...r, profiles: profMap.get(r.partner_id) ?? null })),
      );
      setPartnerFreeAccounts(
        freeRows.map((r: any) => ({ ...r, profiles: profMap.get(r.partner_id) ?? null })),
      );
    })();
  };
  ...
  const addPartner = async () => {
    ...
    const { data: userId, error } = await supabase.rpc("assign_partner_role", {
      _email: email,
  ...
  const saveCommissionRate = async () => {
    ...
    if (codeChanged) {
      ...
      const { error: codeErr } = await supabase.rpc("update_partner_promo_code", {
        _partner_profile_id: editingPartner.id,
        _new_code: customCode,
      });
      if (codeErr) {
        setPartnerSaving(null);
  ...
  const deletePartner = async (p: any) => {
    if (!confirm(`Delete partner ${p.profiles?.full_name ?? p.promo_code}? This cannot be undone.`))
      return;
    setPartnerSaving(p.id);
    const { error } = await supabase.rpc("delete_partner_role" as any, {
      _partner_profile_id: p.id,
    });
    setPartnerSaving(null);
    if (error) return toast.error(error.message);
  ...
  const submitDeliverPartnerFree = async () => {
    ...
    if (
      !partnerFreeForm.login.trim() ||
      !partnerFreeForm.password.trim() ||
      !partnerFreeForm.server.trim()
    ) {
  ...
  const submitDeliverClaim = async () => {
    ...
    if (!chMatch) {
      ...
      return toast.error("No matching challenge found for this account size.");
    }
    const { error } = await supabase
      .from("affiliate_free_accounts")
      .update({
